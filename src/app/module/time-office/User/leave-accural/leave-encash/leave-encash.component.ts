...
import { LeaveEncashesTimeoffice, saveLeaveEncashmentTimeoffice } from 'src/app/store/model/attendance-process.model';
import { LeaveEncashment } from 'src/app/store/model/payroll-action.model';

@Component({
  selector: 'app-leave-encash',
  templateUrl: './leave-encash.component.html',
  styleUrls: ['./leave-encash.component.scss']
})
export class LeaveEncashComponent implements OnInit {
  @Output() multipleLeavediv = new EventEmitter<boolean>();
  @Output() multipleLeaveEncashDiv = new EventEmitter<boolean>();
  ...
  public leaveEncashInfo={} as saveLeaveEncashmentTimeoffice;
  public leaveEncashdtInfo={} as LeaveEncashesTimeoffice;
  ...
  ngOnInit(): void {
    ...
    this.columnDefs=this.userAttendanceService.LeaveEncashprepareColumnForGrid();
  }
  getEncashRequestDetail(){
    var leaveyear=this.paidyear;
    this.userAttendanceService.fetchLeaveEncashDetail(this.paidStatus,leaveyear).subscribe(res=>{
  ...
  saveleaveEncashRequest(){
    this.leaveEncashInfo.remark=this.requestRemark;
    var month = new Date(`${this.leavemonth} 1, ${this.leaveyear}`).getMonth() + 1;
    var monthNumber = month >9 ? month:'0'+month
    this.leaveEncashInfo.payMonthYear=this.leaveyear + '-' + monthNumber + '-01T00:00:00';
    ...
    this.leaveEncashInfo.leaveEncashes=daaa;
    console.log(this.leaveEncashInfo)
    this.userAttendanceService.saveLeaveEncashForEmployee(this.leaveEncashInfo);
  ...
  checkUnCheckAllClicked(chbSelectAll){
   ...
   CancelLevelproces()
   {
   this.multipleLeaveEncashDiv.emit(false);
   }
}
